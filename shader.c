...
////////////////////////////////////////////
	...
	vec_float4 A_dy32 = spu_mul(muls32,A_dy);
	A_dy32 = spu_nmsub(pixels_wide,A_dx,A_dy32);
	...
	int count = (block_bottom+1-block_top)*(block_right+1-block_left);

	by=block_top;
	step = step_start;
	bx = block_left;
	while (count) {
		while (spu_extract(step,0)>=0) {
...
//			bx++;
			step = spu_add(step, spu_splats(-1));
			count--;
		}
		if (by>=cy)
			cx = -1;
